...
export function drawGame(state: State) {
  ...
  if (!state.player.processMovement(currentFrameTime)) {
    if (
      state.keysDown.ArrowUp && state.player.tileFrom[1] > 0 && 
      gameMap[state.player.tileFrom[1] - 1][state.player.tileFrom[0]] === 1
    ) {
      state.player.tileTo[1] -= 1;
    } else if (
      state.keysDown.ArrowDown && state.player.tileFrom[1] < mapHeight - 1 && 
      gameMap[state.player.tileFrom[1] + 1][state.player.tileFrom[0]] === 1
    ) {
      state.player.tileTo[1] += 1;
    } else if (
      state.keysDown.ArrowLeft && state.player.tileFrom[0] > 0 && 
      gameMap[state.player.tileFrom[1]][state.player.tileFrom[0] - 1] === 1
    ) {
      state.player.tileTo[0] -= 1;
    } else if (
      state.keysDown.ArrowRight && state.player.tileFrom[0] < mapWidth - 1 && 
      gameMap[state.player.tileFrom[1]][state.player.tileFrom[0] + 1] === 1
    ) {
      state.player.tileTo[0] += 1;
    }
    ...
    if (
      state.player.tileFrom[0] !== state.player.tileTo[0] || 
      state.player.tileFrom[1] !== state.player.tileTo[1]
    ) {
